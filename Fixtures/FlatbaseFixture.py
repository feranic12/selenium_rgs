...
class FlatbaseFixture(BaseFixture):
    ...
    def registration(self):
        ...
        element = driver.find_element(*FlatbaseLocators.Region_Input)
        #element.click()
        #element.send_keys(Keys.ARROW_DOWN)
        #element.send_keys(Keys.ENTER)
        element.send_keys("город.Москва"+Keys.ENTER)
        element = driver.find_element(*FlatbaseLocators.City_Input)
        element.send_keys("Москва")
        element = driver.find_element(*FlatbaseLocators.Street_Input)
        element.send_keys("Большая почтовая")
        element = driver.find_element(*FlatbaseLocators.House_Input)
        element.send_keys("77")
        element = driver.find_element(*FlatbaseLocators.Corp_Input)
        element.send_keys("2")
        element = driver.find_element(*FlatbaseLocators.Building_Input)
        element.send_keys("3")
